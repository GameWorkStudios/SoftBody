...
public class PressurableThread : MonoBehaviour
{
    ...
    public IEnumerator ExampleCoroutine() {
        isrunning = true;
            ...
            yield return null;
            while(true) {
            if(Mathf.Abs(tr.position.y - lastYValue) > 1f){
                for(int i = 0; i < this.particles.Length; i++){
                    Vector3 vertexPositionForForce = this.particles[i].currentVertexPosition; 
                    forceDirection =  (tr.position - tr.TransformPoint(vertexPositionForForce)).normalized;
                    UpdateVertices(i);
                }
                lastYValue = tr.position.y;
